...
import { getYYYYMMDD, formatTimeFriendly, openModal, renderIssues } from './ui.js';
...
export function renderProfile() {
    ...
    container.innerHTML = `
        <div class="card" style="margin-bottom: 20px;">
            ...
            <button class="btn btn-outline btn-small" style="width: 100%;" onclick="openProfileModal()">Edit Profile Name</button>
        </div>
    `;
}
...
export function renderHome() {
    ...
    schedule.sort((a,b) => a.scheduleTime.localeCompare(b.scheduleTime));
    ...
    schedule.forEach(item => {
        ...
        if(item.status === 'Missed') statusClass = 'status-missed';
        if(item.status === 'Pending-Overdue') statusClass = 'status-missed'; // Visual red border
...
export function renderMeds() {
    ...
    meds.forEach(m => {
        ...
        div.innerHTML = `
            ...
            <p style="font-size:13px; color:var(--text-muted)">Dates: ${m.start_date} to ${m.end_date}</p>
...
export function renderAnalytics() {
    ...
    let chartData = days.map(day => {
        ...
        return { day: day.substring(5).replace('-','/'), taken, missed, total };
    });
...
export function renderManageChildren() {
    ...
    state.children.forEach(c => {
        ...
        div.innerHTML = `<span>${c.name} (${c.age || 'N/A'})</span> <button class="btn btn-red btn-small" onclick="window.deleteChild('${c.id}')">Delete</button>`;
        container.appendChild(div);
    });
}
