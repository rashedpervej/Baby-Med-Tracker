...
export async function clearData() {
    customConfirm("Are you ABSOLUTELY sure? This will delete all children, medicines, issues, and history from the database.", async () => {
        customConfirm("Press OK to permanently delete everything.", async () => {
            const { data: { user } } = await supabaseClient.auth.getUser();
            if (!user) return;
            const uid = user.id;
            ...
            try {
                ...
                await Promise.all([
                    supabaseClient.from('logs').delete().eq('user_id', uid),
                    supabaseClient.from('medicines').delete().eq('user_id', uid),
                    supabaseClient.from('issues').delete().eq('user_id', uid),
                    supabaseClient.from('children').delete().eq('user_id', uid)
                ]);
                localStorage.removeItem('babyMedTrackerSettings');
                localStorage.removeItem('babyMedTrackerActiveChild');
                location.reload();
            } catch (e) {
                console.error(e);
                customAlert("Failed to clear data.");
            } finally {
                showLoading(false);
            }
        }, "Final Warning");
