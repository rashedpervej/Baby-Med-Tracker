...
export function openIssueModal(editId = null) {
    ...
    if (editId) {
        ...
        const isNewMeet = meetIdField.value === 'new';
        
        title.innerText = isNewMeet ? "Add Follow-up Visit" : "Edit Issue";
        ...
        if (isNewMeet) {
            ...
            if (allIssueMeds.length > 0) {
                ...
                prevMedsList.innerHTML = allIssueMeds.map(m => `
                    <label style="display: flex; align-items: center; gap: 5px; background: rgba(var(--primary-rgb), 0.05); padding: 5px 10px; border-radius: 20px; font-size: 12px; cursor: pointer;">
                        <input type="checkbox" class="reuse-med-checkbox" value="${m.id}">
                        ${m.name}
                    </label>
                `).join('');
            } else {
            ...
            // Reset meet-id if just editing the issue
            meetIdField.value = "";
            doctorField.value = latestMeet.doctor_name || '';
            centerField.value = latestMeet.medical_center || '';
    ...
    let content = `
        <div style="text-align: left;">
            ...
            <div style="display: flex; gap: 10px; margin-bottom: 20px;">
                <button class="btn btn-primary btn-small" onclick="window.openFollowUpAction('${issue.id}')">👉 Add Follow-up Visit</button>
            ...
            <div style="display: flex; flex-direction: column; gap: 15px; margin-bottom: 25px;">
                ${issue.meets.map((meet, idx) => {
                    ...
                    return `
                        <div class="card" style="padding: 15px; background: ${idx % 2 === 0 ? 'rgba(var(--primary-rgb), 0.03)' : 'white'}; border-left: 4px solid var(--primary);">
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                                <span class="badge" style="background: var(--primary); color: white;">${idx + 1}${getOrdinalSuffix(idx + 1)} Meet</span>
                                <span style="font-size: 12px; color: var(--text-muted);">${formatDateDisplay(meet.date)}</span>
                            </div>
                            
                            <div style="font-size: 13px; margin-bottom: 10px;">
                                ${meet.doctor_name ? `<div><strong>Doctor:</strong> ${meet.doctor_name}</div>` : ''}
                                ${meet.medical_center ? `<div><strong>Center:</strong> ${meet.medical_center}</div>` : ''}
                            </div>

                            ${meet.notes ? `
                                <div style="background: rgba(0,0,0,0.02); padding: 8px; border-radius: 6px; margin-bottom: 10px; font-size: 13px;">
                                    <strong>Instructions:</strong> ${meet.notes}
                                </div>
                            ` : ''}

                            ${meet.prescriptions && meet.prescriptions.length > 0 ? `
                                <div style="margin-bottom: 10px;">
                                    <p style="font-size: 11px; font-weight: 600; margin-bottom: 5px;">Prescriptions:</p>
                                    <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                                        ${meet.prescriptions.map(url => `
                                            <img src="${url}" style="width: 60px; height: 60px; object-fit: cover; border-radius: 6px; cursor: zoom-in;" onclick="window.zoomImage(this.src)">
                                        `).join('')}
                                    </div>
                                </div>
                            ` : ''}
                            ...
                            <div style="font-size: 12px;">
                                ...
                                <div style="display: flex; flex-wrap: wrap; gap: 5px; margin-top: 5px;">
                                    ${meetMeds.length ? meetMeds.map(m => `
                                        <span class="badge badge-outline" style="font-size: 10px;">${m.name}</span>
...
export function openFollowUpAction(issueId) {
    ...
    // Set meet-id to 'new' to trigger new meet creation in saveIssue
    document.getElementById('issue-meet-id').value = 'new';
